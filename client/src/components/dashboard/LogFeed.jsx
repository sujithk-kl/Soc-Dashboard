...
const LogEntry = ({ log, onClick }) => {
    ...
    return (
        <div
            className="flex gap-4 py-3 border-b border-border last:border-b-0 cursor-pointer hover:bg-dark-gray/20 px-2"
            onClick={() => onClick(log)}
        >
            <div
                className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${severityClasses[log.severity]}`}
            >
    ...
    return (
        <Card title="Real-Time Security Logs">
            <div ref={logContainerRef} className="h-[400px] overflow-y-auto">
                {logs.length > 0 ? (
                    logs.map((log) => (
                        <LogEntry key={log.id} log={log} onClick={handleLogClick} />
                    ))
                ) : (
                    <p className="text-gray-text p-4 text-center">
                        Waiting for new security events...
                    </p>
                )}
            </div>
